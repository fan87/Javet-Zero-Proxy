...
import com.caoccao.javet.interop.proxy.JavetReflectionProxyFactory;
...
// Note from fan87: Had to make scored executable public
// Modified: made it public, and added JSMethodAccessor
 ...
 * @since 0.9.6
 */
public final class CustomScoredExecutable<E extends AccessibleObject> {
    private static final Class<?> V8_VALUE_CLASS = V8Value.class;
    public final E executable;
    private final JSMethodAccessor invoker;
    ...
    public Object execute() throws Throwable {
        ...
        if (length == 0) {
            if (isExecutableVarArgs) {
                Object[] varObject = new Object[0];
                if (executable instanceof Constructor) {
                    return invoker.invoke(null, varObject);
                } else {
                    return invoker.invoke(callee, varObject);
                }
            } else {
                if (executable instanceof Constructor) {
                    return invoker.invoke(null, new Object[0]);
                } else {
                    return invoker.invoke(callee, new Object[0]);
                }
            }
        } else {
            ...
            if (executable instanceof Constructor) {
                return invoker.invoke(null, parameters.toArray());
            } else {
                return invoker.invoke(callee, parameters.toArray());
            }
        }
    }
